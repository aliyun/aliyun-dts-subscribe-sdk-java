...
/**
 * A client that consumes schemaful records from a Togo cluster.
 * <p>
 * This client transparently handles the failure of Togo brokers, and transparently adapts as topic partitions
 ...
 * Togo maintains a numerical offset for each record in a partition. This offset acts as a unique identifier of
 ...
 * <p>
 * Togo uses the concept of <i>consumer groups</i> to allow a pool of processes to divide the work of consuming and
 ...
 * {@link #subscribe(Collection, ConsumerRebalanceListener) subscribe} APIs. Togo will deliver each message in the
 ...
 * multiple processes. As a multi-subscriber system, Togo naturally supports having any number of consumer groups for a
 ...
 * commits, etc. See <a href="#rebalancecallback">Storing Offsets Outside Togo</a> for more details.
 * <p>
 ...
 * This example demonstrates a simple usage of Togo's consumer api that relying on automatic offset committing.
 * <p>
 * <pre>
 *     {@code
 ...
 *     TogoConsumer consumer = new TogoConsumer(props);
 *     consumer.subscribe(Arrays.asList("foo", "bar"));
 *     while (true) {
 *         SchemafulConsumerRecords records = consumer.poll(100);
 *         for (SchemafulConsumerRecord record : records) {
 *             System.out.printf("offset = %d, schema = %s, value = %s%n", record.offset(),
 *                 record.schemaAndValue().getSchema(),
 *                 record.schemaAndValue().getValueAsRecord());
 *         }
 *     }
 *     }
 * </pre>
 ...
 *     TogoConsumer<String, String> consumer = new TogoConsumer(props);
 *     consumer.subscribe(Arrays.asList("foo", "bar"));
 *     final int minBatchSize = 200;
 *     List<SchemafulConsumerRecord> buffer = new ArrayList<>();
 *     while (true) {
 *         SchemafulConsumerRecords records = consumer.poll(100);
 *         for (SchemafulConsumerRecord record : records) {
 *             buffer.add(record);
 ...
 * Togo provides what is often called "at-least-once" delivery guarantees, as each record will likely be delivered one
 ...
 * <p>
 * In the previous examples, we subscribed to the topics we were interested in and let Togo dynamically assign a
 ...
 * this case there is no need for Togo to detect the failure and reassign the partition since the consuming process
 ...
 * <p>
 * <h4><a name="rebalancecallback">Storing Offsets Outside Togo</h4>
 * <p>
 * The consumer application need not use Togo's built-in offset storage, it can store offsets in a store of its own
 * choosing. The primary use case for this is allowing the application to store both the offset and the results of the
 * consumption in the same system in a way that both the results and offsets are stored atomically. This is not always
 * possible, but when it is it will make the consumption fully atomic and give "exactly once" semantics that are
 * stronger than the default "at-least once" semantics you get with Togo's offset commit functionality.
 * <p>
 ...
 * position (either automatically or manually). However Togo allows the consumer to manually control its position,
 ...
 * re-consuming all the data and recreating the state (assuming that Togo is retaining sufficient history).
 * <p>
 * Togo allows specifying the position using {@link #seek(TopicPartition, long)} to specify the new position. Special
 ...
 * <p>
 * <p>
 * Togo supports dynamic controlling of consumption flows by using {@link #pause(Collection)} and {@link #resume(Collection)}
 * to pause the consumption on the specified assigned partitions and resume the consumption
 * on the specified paused partitions respectively in the future {@link #poll(long)} calls.
 * <p>
 * <h3><a name="multithreaded">Multi-threaded Processing</a></h3>
 * <p>
 * The Togo consumer is NOT thread-safe. All network I/O happens in the thread of the application
 ...
 * <p>
 * <pre>
 * public class TogoConsumerRunner implements Runnable {
 *     private final AtomicBoolean closed = new AtomicBoolean(false);
 *     private final TogoConsumer consumer;
 *
 *     public void run() {
 *         try {
 ...
 * is impossible, e.g. when a consumer thread is managed by code that is unaware of the Togo client.
 * <p>
 * <p>
 ...
 * <li><b>CON</b>: More consumers means more TCP connections to the cluster (one per thread). In general Togo handles
 ...
 * commit.
 */
public class TogoConsumer implements SchemafulConsumer {
    private final static Logger logger = LoggerFactory.getLogger(TogoConsumer.class);

    private final TogoAdaptedKafkaConsumer innerConsumer;
    private final SchemafulConsumerInterceptors interceptors;

    public TogoConsumer(Properties properties) {
        this(new SchemafulConsumerConfig(properties));
    }

    public TogoConsumer(Map<String, Object> configs) {
        this(new SchemafulConsumerConfig(configs));
    }

    private TogoConsumer(SchemafulConsumerConfig config) {
        Map<String, Object> originals = config.originals();
        String interceptorClasses = (String) originals.get(SchemafulConsumerConfig.INTERCEPTOR_CLASSES_CONFIG);
        if (interceptorClasses != null) {
            originals.put(SchemafulConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, ConsumerInterceptorCommitAdapter.class.getName());
            originals.put(ConsumerInterceptorCommitAdapter.INNER_CONSUMER_CONFIG, this);
        }
        this.innerConsumer = new TogoAdaptedKafkaConsumer(preProcessProperties(originals));

        if (interceptorClasses != null) {
            originals.put(ConsumerConfig.CLIENT_ID_CONFIG, innerConsumer.getClientId());
            originals.put(SchemafulConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, interceptorClasses);
            List<SchemafulConsumerInterceptor> interceptorList = (new SchemafulConsumerConfig(originals))
                    .getConfiguredInstances(SchemafulConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, SchemafulConsumerInterceptor.class);
            this.interceptors = new SchemafulConsumerInterceptors(interceptorList == null ? new ArrayList<>() : interceptorList);
        } else {
    ...
    @Override
    public SchemafulConsumerRecords poll(long timeout) {
        SchemafulConsumerRecords records = innerConsumer.pollSchemaful(timeout);
        if (interceptors != null) {
            return interceptors.onConsume(records);
        } else {
            return records;
        }
    }
    ...
    @Override
    public void setFetchRule(TopicPartition topicPartition, FetchRule fetchRule) {
        Objects.requireNonNull(topicPartition);
        Objects.requireNonNull(fetchRule);

        innerConsumer.setFetchRule(topicPartition, fetchRule);
    }

    @Override
    public void setFetchMatchFunction(TagMatchFunction tagMatchFunction) {
        Objects.requireNonNull(tagMatchFunction);
        innerConsumer.setTagMatchFunction(tagMatchFunction);
    }

    public void addClusterResourceListener(ClusterResourceListener listener) {
        innerConsumer.addClusterResourceListeners(listener);
    }
    ...
    public Set<TopicPartition> assignment() {
        return innerConsumer.assignment();
    }
    ...
    public Set<String> subscription() {
        return innerConsumer.subscription();
    }
    ...
    public void subscribe(Collection<String> topics, ConsumerRebalanceListener callback) {
        innerConsumer.subscribe(topics, callback);
    }
    ...
    public void subscribe(Pattern pattern, ConsumerRebalanceListener callback) {
        innerConsumer.subscribe(pattern, callback);
    }
    ...
    public void assign(Collection<TopicPartition> partitions) {
        innerConsumer.assign(partitions);
    }
    ...
    public void commitSync(Map<TopicPartition, OffsetAndMetadata> offsets) {
        innerConsumer.commitSync(offsets);
    }
    ...
    public void commitAsync(OffsetCommitCallback callback) {
        innerConsumer.commitAsync(callback);
    }
    ...
    public void commitAsync(Map<TopicPartition, OffsetAndMetadata> offsets, OffsetCommitCallback callback) {
        innerConsumer.commitAsync(offsets, callback);
    }
    ...
    public void seek(TopicPartition partition, long offset) {
        innerConsumer.seek(partition, offset);
    }
    ...
    public void seekToBeginning(Collection<TopicPartition> partitions) {
        innerConsumer.seekToBeginning(partitions);
    }
    ...
    public void seekToEnd(Collection<TopicPartition> partitions) {
        innerConsumer.seekToEnd(partitions);
    }
    ...
    public long position(TopicPartition partition) {
        return innerConsumer.position(partition);
    }
    ...
    public OffsetAndMetadata committed(TopicPartition partition) {
        return innerConsumer.committed(partition);
    }
    ...
    public List<PartitionInfo> partitionsFor(String topic) {
        return innerConsumer.partitionsFor(topic);
    }
    ...
    public Map<String, List<PartitionInfo>> listTopics() {
        return innerConsumer.listTopics();
    }
    ...
    public void pause(Collection<TopicPartition> partitions) {
        innerConsumer.pause(partitions);
    }
    ...
    public void resume(Collection<TopicPartition> partitions) {
        innerConsumer.resume(partitions);
    }
    ...
    public Map<TopicPartition, OffsetAndTimestamp> offsetsForTimes(Map<TopicPartition, Long> timestampsToSearch) {
        return innerConsumer.offsetsForTimes(timestampsToSearch);
    }

    /**
     * Look up the offsets for the given partitions by query conditions. The returned offset for each partition is the
     * earliest offset that greater the condition value respectively.
     * <p>
     * This is a blocking call. The consumer does not have to be assigned the partitions.
     * <p>
     * Notice that this method may block indefinitely if the partition does not exist.
     *
     * @param queriesToSearch the mapping from partition to the timestamp to look up.
     * @return a mapping from partition to the timestamp and offset of the first message matches the condition.
    ...
    public Map<TopicPartition, OffsetAndQuery> offsetsForFirstMatch(Map<TopicPartition, QueryCondition> queriesToSearch) {
        return innerConsumer.offsetsForFirstMatch(queriesToSearch);
    }
    ...
    public Map<TopicPartition, Long> beginningOffsets(Collection<TopicPartition> partitions) {
        return innerConsumer.beginningOffsets(partitions);
    }
    ...
    public Map<TopicPartition, Long> endOffsets(Collection<TopicPartition> partitions) {
        return innerConsumer.endOffsets(partitions);
    }
    ...
    @Override
    public void wakeup() {
        innerConsumer.wakeup();
    }

    public static class ConsumerInterceptorCommitAdapter implements ConsumerInterceptor<byte[], byte[]> {
        static final String INNER_CONSUMER_CONFIG = "togo.inner.interceptors";
        private TogoConsumer consumer;

        @Override
        public void configure(Map<String, ?> configs) {
            logger.debug("To configure the interceptor adapter with {}", configs);
            this.consumer = (TogoConsumer) configs.get(INNER_CONSUMER_CONFIG);
            if (consumer == null) {
                throw new IllegalStateException("No consumer instance provided for the adapter!");
            }
        }
        ...
        public void onCommit(Map<TopicPartition, OffsetAndMetadata> offsets) {
            this.consumer.interceptors.onCommit(offsets);
        }
