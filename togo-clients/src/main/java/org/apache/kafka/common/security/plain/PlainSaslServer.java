...
public class PlainSaslServer implements SaslServer {
    ...
    public byte[] evaluateResponse(byte[] response) throws SaslException, SaslAuthenticationException {
        ...
        if (!jaasContext.configurationEntries().isEmpty()
                && jaasContext.configurationEntries().get(0).getOptions().containsKey(SIDUtil.JAAS_INTERCEP_TAG)) {
            String[] userNameAndSid = username.split(SIDUtil.SIDSplitChar, 2);
            if (userNameAndSid.length != 2) {
                throw new SaslAuthenticationException("Authentication failed: Invalid username or password, username:" + username);
            } else {
                this.authorizationId = SIDUtil.encode(userNameAndSid[0], userNameAndSid[1], password);
                isEncoded = true;
                complete = true;
                return new byte[0];
            }
