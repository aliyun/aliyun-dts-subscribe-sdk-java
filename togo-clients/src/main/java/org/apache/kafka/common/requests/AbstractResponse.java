    ...
    public static AbstractResponse parseResponse(ApiKeys apiKey, Struct struct, short version) {
        switch (apiKey) {
            ...
            case CREATE_RECORD_METAS:
                return new CreateRecordMetasResponse(struct);
            case FETCH_RECORD_METAS:
                return new FetchRecordMetasResponse(struct);
            case FLEX_LIST_OFFSET:
                return new FlexListOffsetResponse(struct);
            case SET_FETCH_TAGS:
                return new SetFetchTagsResponse(struct);
            case RELOAD_AUTH_META:
                return new ReloadAuthMetaResponse(struct);
            default:
