...
public class MemoryRecordsBuilder {
    ...
    private Long appendWithOffset(long offset, boolean isControlRecord, long timestamp, ByteBuffer key,
        ...
        try {
            ...
            if (magic < RecordBatch.MAGIC_VALUE_V2 && headers != null && headers.length > 0) {
                // this will cause togo integrate test integration.kafka.api.PlaintextTogoConsumerTest#testEarliestOrLatestOffsets failed
                // mute this exception will pass the test.
                // may be the designer don't want miss the header info silently.
     ...
     * Append a log record using a different offset
     * @param offset The offset of the record
     * @param timeStamp The reset timestamp for record
     * @param record The record to add
     */
    public void appendWithOffset(long offset, long timeStamp, Record record) {
        appendWithOffset(offset, timeStamp, record.key(), record.value(), record.headers());
    }
