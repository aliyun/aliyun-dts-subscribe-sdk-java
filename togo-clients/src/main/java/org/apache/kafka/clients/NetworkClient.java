...
public class NetworkClient implements KafkaClient {
    ...
    private final List<String> metaDataProperties;

    private final Sensor throttleTimeSensor;

    /**
     *  if one node always connect failed in certain times, we should interrupt the retry logic
     */
    private final Map<String, Pair<Long, Integer>> disconnectCounter;
    ...
    public NetworkClient(Selectable selector,
        ...
        this(null, metadata, selector, clientId, maxInFlightRequestsPerConnection,
             ...
             discoverBrokerVersions, apiVersions, null, logContext, null);
    }
    ...
    public NetworkClient(Selectable selector,
        ...
        this(null, metadata, selector, clientId, maxInFlightRequestsPerConnection,
                reconnectBackoffMs, reconnectBackoffMax, socketSendBuffer, socketReceiveBuffer, requestTimeoutMs, time, discoverBrokerVersions,
                apiVersions, throttleTimeSensor, logContext, metaDataProperties);
    }
    ...
    public NetworkClient(Selectable selector,
        ...
        this(null, metadata, selector, clientId, maxInFlightRequestsPerConnection,
             reconnectBackoffMs, reconnectBackoffMax,
             socketSendBuffer, socketReceiveBuffer, requestTimeoutMs, time,
             discoverBrokerVersions, apiVersions, throttleTimeSensor, logContext, null);
    }
    ...
    private NetworkClient(MetadataUpdater metadataUpdater,
        ...
        this.disconnectCounter = new HashMap<>();
    }
    ...
    public static AbstractResponse parseResponse(ByteBuffer responseBuffer, RequestHeader requestHeader) {
        ...
        return AbstractResponse.parseResponse(requestHeader.apiKey(), responseStruct, requestHeader.apiVersion());
    }
    ...
    private void processDisconnection(List<ClientResponse> responses, String nodeId, long now, ChannelState disconnectState) {
        ...
        switch (disconnectState.state()) {
            ...
            case AUTHENTICATE:
                ...
                break;
            case NOT_CONNECTED:
                disconnectCounter.compute(nodeId, (nid, pair) -> {
                    if (null == pair) {
                        pair = Pair.of(0L, 1);
                    }
                    Long currentTimeMS = System.currentTimeMillis();
                    if (currentTimeMS - pair.getLeft() > reconnectBackoffMs * 10) {
                        // reset counter cause next connect interval is bigger than safe quota
                        pair = Pair.of(currentTimeMS, 1);
                    } else {
                        // increment retry time
                        int updatedCounter = pair.getRight() + 1;
                        pair = Pair.of(currentTimeMS, updatedCounter);
                    }
                    return pair;
                });
                Pair<Long, Integer> retryCounter = disconnectCounter.get(nodeId);
                Preconditions.checkState(null != retryCounter);
                log.warn("Connection to node {} could not be established. Broker may not be available. Try time {}, last try timeMS {}", nodeId, retryCounter.getLeft(), retryCounter.getRight());
                if (retryCounter.getRight() * reconnectBackoffMs > requestTimeoutMs) {
                    throw new BrokerNotAvailableException("Connection to node " + nodeId + " could not be established. Broker may not be available. Exceed max try timeMs " + (retryCounter.getRight() * reconnectBackoffMs) + ", please check");
                }
                break;
            default:
    ...
    private void handleCompletedReceives(List<ClientResponse> responses, long now) {
        for (NetworkReceive receive : this.selector.completedReceives()) {
            ...
            AbstractResponse body = AbstractResponse.parseResponse(req.header.apiKey(), responseStruct, req.header.apiVersion());
        ...
        private long maybeUpdate(long now, Node node) {
            ...
            if (canSendRequest(nodeConnectionId)) {
                ...
                if (metadata.needMetadataForAllTopics())
                    metadataRequest = MetadataRequest.Builder.allTopics().defaultConfigs(metaDataProperties);
                else
                    metadataRequest = new MetadataRequest.Builder(new ArrayList<>(metadata.topics()),
                            metadata.allowAutoTopicCreation()).defaultConfigs(metaDataProperties);
